...
QUESTIONS = [
    ...
    {
        "question": "Для чего используется Jinja2?",
        "category": "базовый",
        "expected_keywords": ["jinja", "шаблон", "html"]
    },
    {
        "question": "Что делает команда pytest -v?",
        "category": "средний",
        "expected_keywords": ["pytest", "verbose", "тест"]
    },
]
...
def run_evaluation(agent: Agent):
    ...
    for i, q in enumerate(QUESTIONS, 1):
        ...
        for mode in modes:
            ...
            try:
                ...
                results[mode["name"]] = {"error": str(e), "hits": 0, "total": 0, "text": "", "fallback": False, "latency": 0}
        ...
        has_sources = bool(re.search(r"(?:Sources|Источники):\s*\S", best_text, re.IGNORECASE))
        
        # Ищем кавычки или скобки, но не "НЕТ"
        has_quotes_pattern = bool(re.search(r"(?:Quotes|Цитаты):.*?[\"\[]", best_text, re.DOTALL | re.IGNORECASE))
        has_quotes = has_quotes_pattern and "НЕТ" not in best_text
    ...
    console.print(f"Средний latency: {avg_latency:.2f}s")
    
    # Вывод для парсинга скриптами
    from app.config import CURRENT_CONFIG
    print(f"__CONFIG__:{CURRENT_CONFIG}")
    print(f"__FALLBACK__:{fallback_pct:.0f}")
    print(f"__SOURCES__:{sources_pct:.0f}")
    print(f"__QUOTES__:{quotes_pct:.0f}")
    print(f"__LATENCY__:{avg_latency:.2f}")
