...
def normalize_output(text: str) -> str:
    """Нормализовать вывод для корректного парсинга."""
    text = text.strip()
    text = text.replace("\r\n", "\n")
    return text
...
def parse_response(text: str) -> dict | None:
    ...
    quotes_match = re.search(r"Цитаты:\s*\n?(.+)", text, re.DOTALL | re.IGNORECASE)
    if not quotes_match:
...
LOCAL_PROMPT_TEMPLATE = """Ты — RAG-ассистент. Отвечай ТОЛЬКО на основе контекста.

ПРАВИЛА:
- Не используй знания вне контекста
- Если ответа нет → напиши: Не знаю
- Ответ должен быть кратким (1–2 предложения)
- Строго соблюдай формат ниже
...
Цитаты:
<дословные цитаты или НЕТ>

ПРАВИЛА ДЛЯ ЦИТАТ:
- Только точные фрагменты из контекста
- Не изменяй текст
- Не вставляй код
- Если нет → НЕТ
...
Ответ:
Python Fire — это библиотека для создания CLI.
...
def generate_answer_with_context(question: str, chunks: list, llm_client=None) -> dict:
    ...
    raw_text = normalize_output(raw_result.get("text", ""))
