...
def run_evaluation(agent: Agent):
    ...
    for i, q in enumerate(QUESTIONS, 1):
        ...
        has_quotes = bool(re.search(r"Quotes:\s*\d", best_text, re.IGNORECASE)) or \
                     bool(re.search(r"(?:Цитаты|Quotes):.*?\"", best_text, re.DOTALL | re.IGNORECASE)) or \
                     bool(re.search(r"(?:Цитаты|Quotes):.*?\[", best_text, re.DOTALL | re.IGNORECASE))
