...
class Retriever:
    ...
    def retrieve(self, query: str, k: int = INITIAL_TOP_K) -> list:
        ...
        Returns:
            List of relevant chunks with metadata, distance score, and chunk_id
        """
        ...
        for i, idx in enumerate(indices[0]):
            if idx >= 0 and idx < len(self.metadata):
                ...
                chunk["chunk_id"] = chunk.get("chunk_id", idx)
                results.append(chunk)
        
        return results


def filter_chunks(chunks: list, max_distance: float = MAX_DISTANCE, top_k: int = FINAL_TOP_K) -> tuple:
    """
    Filter chunks by distance threshold and return top-k with dynamic selection.
    ...
    Returns:
        Tuple of (filtered_chunks, final_top_k_used, best_distance)
    """
    if not chunks:
        return [], top_k, float("inf")
    ...
    if not filtered:
        return chunks[:top_k], top_k, best_distance
    
    sorted_chunks = sorted(filtered, key=lambda x: x.get("score", float("inf")))
    
    return sorted_chunks[:dynamic_top_k], dynamic_top_k, best_distance
