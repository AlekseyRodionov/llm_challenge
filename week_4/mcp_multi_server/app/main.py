...
[bold]🔄 MULTI-SERVER PIPELINE[/bold]
  run_multi_pipeline — запустить multi-server pipeline (автоматический режим)
  interactive — запустить интерактивный режим с LLM анализом
    1. fetch_recent_reports (demo_server)
    2. fetch_productivity_tips (external_api_server) — перевод советов
    3. analyze_reports (demo_server)
    4. generate_summary + LLM (demo_server) — анализ и рекомендации
    5. save_summary (demo_server)
""")
...
def main():
    ...
    console.print("[bold]🔄 MULTI-SERVER PIPELINE[/bold]")
    console.print("  run_multi_pipeline, interactive")
    ...
    while True:
        try:
            ...
            if command == "mcp_start":
                ...
                if mcp_client.connect("mcp_servers/demo_server.py"):
            ...
            if command == "run_multi_pipeline":
                console.print("[bold]Starting Multi-Server Pipeline...[/bold]\n")
                from app.orchestrator import MultiServerOrchestrator
            ...
            if command == "interactive":
                console.print("[bold]Starting Interactive Agent...[/bold]\n")
                from app.orchestrator import MultiServerOrchestrator
                orchestrator = MultiServerOrchestrator()
